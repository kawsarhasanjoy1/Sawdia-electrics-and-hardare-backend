...
const createProductController = catchAsync(
  async (req: Request, res: Response) => {
    ...
    const product = await ProductServices.createProduct(data, user, images);
    sendResponse(res, {
      statusCode: 201,
      success: true,
...
const updateProductSaveController = catchAsync(
  async (req: Request, res: Response) => {
    const userId = req.user?.userId;
    const productId = req.params.id;
    const product = await ProductServices.updateProductSave(userId, productId);
    sendResponse(res, {
      statusCode: 200,
      success: true,
...
const restoreProductController = catchAsync(
  async (req: Request, res: Response) => {
