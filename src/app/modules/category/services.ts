...
const createCategory = async (payload: TCategory) => {
    const parentCategory = await ParentCategoryModel.findById({ _id: payload.parentCategory });
    if (!parentCategory) throw new AppError(StatusCodes.NOT_FOUND, 'Parent category not found');
    console.log(payload, parentCategory)
    payload.slug = slugify(payload.slug, { lower: true });
...
const getAllCategories = async () => {
    const categories = await CategoryModel.find({ isDeleted: false }).populate('parentCategory').populate('createdBy', 'name email role');
    return categories;
};
